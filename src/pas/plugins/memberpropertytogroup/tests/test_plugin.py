...
class TestPluginHelpers(unittest.TestCase):
    ...
    def test_group_property_of_principal(self):
        ...
        self.mocked_sheet_plugins.return_value = {"testsheet": MockProvider()}
...
class TestPlugin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mocked_group_property_of_principal = (
            mock_group_property_of_principal.start()
        )  # noqa
        self.addCleanup(mock_group_property_of_principal.stop)

        # mock valid_groups
        mock_valid_groups = mock.patch.object(
            MPTGPlugin, "_valid_groups", mock.Mock(wraps=MPTGPlugin._valid_groups)
        )
        ...
        mock_get_all_groups = mock.patch.object(
            MPTGPlugin, "_get_all_groups", mock.Mock(wraps=MPTGPlugin._get_all_groups)
        )
    ...
    def test_getGroups(self):
        ...
        self.mocked_group_property_of_principal.return_value = ""
        groups = [group for group in self.plugin.getGroups()]
    ...
    def test_enumerateGroups(self):
        ...
        self.mocked_group_property_of_principal.return_value = ""
        self.assertEqual(len(self.plugin.enumerateGroups(id="group")), 2)
        self.assertEqual(len(self.plugin.enumerateGroups()), 2)
        self.assertEqual(len(self.plugin.enumerateGroups(id="group1")), 1)
        self.assertEqual(
            self.plugin.enumerateGroups(id="group1", exact_match=True)[0]["id"],
            "group1",
        )
        self.assertEqual(self.plugin.enumerateGroups(id="group", exact_match=True), [])
